...
class Dataset:
    ...
    def __repr__(self):
        return f'Dataset: {self.__class__.__name__}\n'\
               f'\tTraining size: {len(self.training_dataloader())} samples\n' \
               f'\tValidation size: {len(self.validation_dataloader())} samples\n' \
               f'\tTest size: {len(self.validation_dataloader())} samples'
